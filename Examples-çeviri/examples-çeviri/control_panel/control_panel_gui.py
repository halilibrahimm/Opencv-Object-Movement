...
# Auto_detection kitaplığını içe aktarmaya çalışın 
...
atexit.register(gopigo.stop)    # aktarılacak tüm isteğe bağlı argümanlar, register() için bağımsız değişken olarak aktarılmalıdır.
...
class gopigo_control_app(wx.Frame):#Çerçeve, boyutu ve konumu kullanıcı tarafından değiştirilebilen (genellikle) bir pencere.
    slider=0
    def __init__(self,parent,id,title):
        wx.Frame.__init__(self,parent,id,title,size=(475,600))
        self.parent = parent
        self.initialize()#başlangıç durumuna getir
        # Exit
    ...
    def initialize(self):
        ...
        # Otomatik algılayabilirsek, kullanıcıya geribildirim verir
    ...
    def onClose(self, event):	# Tüm programı kapatın.
        self.Close()
