...
#
########################################################################
# GoPiGo veya GrovePi üzerindeki GPS sensörünün çiğ çıktısı ve çıktısı.
...
import struct
import sys
import ir_receiver_check#ır alıcısı

if ir_receiver_check.check_ir():
	print "Devam etmeden önce IR alıcısını devre dışı bırak"
	exit()

ser = serial.Serial('/dev/ttyAMA0',  9600, timeout = 0)	#Seri portu 9600 baud'da açın
ser.flush()

def readlineCR():
    rv = ""
    while True:
        time.sleep(0.01)	# Bu kritik kısımdır. Küçük bir duraklama
        ch = ser.read()  #seri portu okuma      
        rv += ch#seri porttaki değerleri rv ye atıyor
        if ch=='\r' or ch=='':
            return rv #\r yi görene kadar devam ediyor ve rv yi geri döndürüyor
